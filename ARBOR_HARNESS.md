...
### 1-6. 새 run(러너/풀이 루프)을 만들지 마라 — 단일 메인 run 을 고쳐 개발한다 (2026-07-25 추가)
- 지금 문제를 푸는 run 은 **하나뿐**이다: `arbor/agent.py :: ArborAgent.run()` — `build_agent(task) → ArborAgent`
  가 스스로 도는 focus-descent 풀이 루프. 모든 진입점(`python -m arbor` / `./run.sh` / dashboard / program_report)이
  캐시 래퍼 `debugger/solve_cache.run_solve` 를 거쳐 **이 하나의 run 으로 수렴**한다.
- **금지:** 개발·실험 편의로 별도의 run/러너/솔브 루프를 새로 만드는 것(예: `run2`, `solve_v2`, 평행 파이프라인).
  run 이 둘이 되면 진실의 원천이 갈라져 검증이 무의미해진다(§2-4 와 같은 이유). 새 진입점이 필요해도 이 run 을
  **호출**할 뿐, 새 풀이 루프를 짜지 않는다.
- **요구:** 풀이 흐름을 바꾸려면 이 **메인 run(`ArborAgent.run`)을 직접 수정·확장**한다. 새 동작도 이 한 루프 안에서
  operator/production/WM 으로 표현한다. (캐시 래퍼 `run_solve` 는 현재 `debugger/` 에 있고 후속에서 `arbor/` 로
  이관 예정 — 그래도 run 루프 자체는 이 하나다.)
...
- 특히 §1-5 의 탐색 트레이스(시도·기각 후보)와 하강 단계는 dashboard 에서 읽혀야 한다 (근거가 WM 에
  남는다 = 화면에도 남는다).
- **pair.program / TASK.solution 시각화 고정 규칙 (2026-07-26 확정 · 사용자 승인)**:
  `docs/superpowers/specs/2026-07-26-tasksolution-viz-rules.md` 를 따른다. 요지 — ① 데이터플로우 박스(값 아래·함수
  위·선은 가로/세로만) ② placeholder 는 `?p` 뿐, 그 외는 실존 함수명·값·기존 symbol(ARCKG 약자 t·p·g·o·**x**=픽셀)
  ③ **함수조합 arg 는 `?p` 로 윗줄 승격**(한 함수 줄엔 arg 개수만큼) ④ 좌표 튜플 `( , )` 는 infix ⑤ `?p` 전역
  유일번호 ⑥ TASK.solution = **골격 1장 + 각 `?p`/이름 정의 N장 분리**(태스크별 독립). 렌더/표현을 고칠 때 이 규칙을
  어기지 않는다.
...
- [ ] 새 run/러너/풀이 루프를 새로 만들고 있지 않은가? 단일 메인 run(`ArborAgent.run`)을 고쳐 개발하는가? (§1-6)
