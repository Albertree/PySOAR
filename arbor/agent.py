"""
ARBOR agent — the ARBOR agent that OWNS its solve loop.

  build_agent(task) -> ArborAgent   # soar 코어(self.ag)를 품고 focus-descent 풀이 루프 소유
  ArborAgent.run(sink=NullSink())    # 스스로 돈다: 빠름(기록 0) / run()=JournalSink 기록→dashboard

soar.Agent(=self.ag) 는 안쪽 엔진(한 사이클=step, 범용·오라클검증). ARBOR 고유 오케스트레이션
(focus 하강·2-사이클 ONC·env 3회 재시도)은 여기(ArborAgent)에 있고 커널엔 안 섞는다.

run 루프는 풀이과정을 ATOMIC step 으로 쪼개 매 변화마다 sink.event() 로 방출한다(input ->
propose -> decide -> apply -> output, propose/apply 안엔 elaboration round, impasse 엔 substate).
NullSink 이면 방출 no-op(빠른 채점), JournalSink 면 기록 → renderer(arbor/trace) → dashboard.
...
from arbor.soar import Agent  # noqa: E402
from arbor.soar.decide import run_preference_semantics  # noqa: E402
from arbor.soar.preference import SYMBOL_TO_TYPE, PreferenceType  # noqa: E402
from arbor.procedural_memory.loader import PRODUCTIONS  # noqa: E402
from arbor.procedural_memory.operators import OPERATOR_BODIES  # noqa: E402
from arbor.perception.nav import index_arckg  # noqa: E402
from arbor.expr_solver import build_arckg  # noqa: E402
...
def inject_perception(ag):
    ...
    ag.wm.add("S1", "to-observe", "yes")              # 관측할 게 있음 → observe 제안 → impasse → select


def _build_kernel(task, tid="0a", record=False):
    """soar 코어 Agent 인스턴스 + ARBOR 규칙/operator/지각(inject_perception) 배선."""
...
class ArborAgent:
    """ARBOR 에이전트: soar 코어(self.ag)를 품고 focus-descent 풀이 루프를 소유한다.
    build 후 스스로 돈다 — run(sink=NullSink())=빠름 / run()=JournalSink 기록→dashboard."""
    def __init__(self, task, tid="0a", record=False):
        self.ag = _build_kernel(task, tid, record)  # 안쪽 SOAR 커널 엔진(한 사이클=step)
        self.sink = None                            # run(sink=...) 에서 설정
    ...
    def _render(self):
        if self._rendered is None:
            from arbor.trace.renderer import render
    ...
    def _match_preview(self):
        ...
        from arbor.soar.production import match
        from arbor.soar.elaborate import instantiate
    ...
    def run(self, sink=None, max_cycles=10):
        from arbor.trace.sink import JournalSink
        ...
        for _ in range(max_cycles):
            ...
            from arbor.soar.decide import ImpasseType
            # ── A안: 2-사이클 충실 ONC 감지 ────────────────────────────────────
...
def build_agent(task, tid="0a", record=False):
    """ARBOR 에이전트 팩토리: build 하면 스스로 run(sink) 할 수 있는 ArborAgent 를 돌려준다."""
    return ArborAgent(task, tid, record)
