...
def make_dashboard(tasks, dataset="focus (slice 1)", out_name="focus_dashboard.html",
                   report_href="program_report_all.html"):
    ...
    out = os.path.join(_REPO, "debugger", "traces", out_name)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "move":
        ...
        from debugger.reports.program_viewer import build_move as _build_mv
        mv = _build_mv()
        print(f"wrote {mv}\nopen it:  open {out}")
    else:
