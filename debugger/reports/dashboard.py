...
def _safe_dash_data(task, tid, timeout_s=180):   # 제출 예산과 동일한 문제당 3분
    ...
    if not hasattr(signal, "SIGALRM"):                   # Windows: SIGALRM 없음 → 타임아웃 없이 예외격리만
        try:                                             # (max_cycles 가 런타임을 bound 하므로 무한대기 없음)
            return _dash_data(task, tid)
...
def make_dashboard(tasks, dataset="focus (slice 1)", out_name="dashboard.html",
                   report_href="program_report.html"):
    """tasks: [(tid, task_dict), ...] — 대시보드 TASK BROWSER 에 카드로 나열.
    out_name/report_href 로 다른 데이터셋(예: move)용 대시보드도 같은 구성으로 생성."""
    ...
    # 상단 nav 링크(고정): 각 태스크의 PAIR.program(text+AST+시각화)+TASK.solution 을 확인하는
    # program 뷰어로 이동. 공유 _HTML 를 오염시키지 않으려 출력에만 주입한다.
    nav = (f'<a href="{report_href}" onclick="try{{location.href=\'{report_href}#\''
...
if __name__ == "__main__":
    import sys
    from debugger.reports import program_report
    arg = sys.argv[1] if len(sys.argv) > 1 else None
    if arg == "move":
        # arc_human/move 전용: move_dashboard.html + move_program_report.html (동일 구성)
        out = build_move_dashboard()
        print(f"wrote {out}  ({os.path.getsize(out) / 1e6:.1f} MB)")
        mv = program_report.build_move()
        print(f"wrote {mv}\nopen it:  open {out}")
    elif arg == "objc":
        # object_coloring 전용: objc_dashboard.html + objc_program_report.html (동일 구성)
        out = build_objc_dashboard()
        print(f"wrote {out}  ({os.path.getsize(out) / 1e6:.1f} MB)")
        oc = program_report.build_objc()
        print(f"wrote {oc}\nopen it:  open {out}")
    elif arg == "rotate":
        out = build_rotate_dashboard()
        print(f"wrote {out}  ({os.path.getsize(out) / 1e6:.1f} MB)")
        rt = program_report.build_rotate()
        print(f"wrote {rt}\nopen it:  open {out}")
    else:
