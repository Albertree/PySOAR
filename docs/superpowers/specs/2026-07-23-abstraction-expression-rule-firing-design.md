...
- **하강의 통일 조건 = verify: "현재 (부분 가능) program 이 그 pair 의 output G1 을 생성하는가?"**
  program 은 부분일 수 있다(grid comparison 이 정한 size/color 는 stash=grid-skeleton, object/pixel 이
  이어서 채움). verify 가 그 **현재 program 을 실행기(C2)로 실행해 G1 과 대조** → 맞으면 그 pair 완성,
  **못 맞추면(빈·부분·부정확 전부) 하강**. 이 실패는 **진짜**다(program 이 실제로 G1 을 못 냄) —
  `hypothesized failed` 는 그 진짜 실패의 결과라 정당하며 **유지**한다(가짜 아님). 계층 무관.
- **object-level 정직화 (가짜/오해 표현만 제거)**:
  - **"원천차단" 주석 제거** — object recolor 를 *일부러 막았다*는 서술은 오해. object→coloring 규칙이
    아직 없어 object 가 program 에 기여를 못 할 뿐.
  - **`colored-all yes` 제거** — 칠한 게 없는데 "coloring 끝남"을 세워 verify 를 억지 발화시키는 거짓
    플래그. verify 는 `colored-all` 이 아니라 "검증할 현재 program 존재"에 걸어 발화(부분 program 이
    늘 있으므로 항상 발화 가능).
  - **`sim`=G0 해킹 제거** — sim 스크래치 대신 실제 (부분) program 을 실행기로 실행해 대조. object 는
    기여분이 없어 G1 과 달라 → 정직하게 verify 실패 → 하강.
  - object→pixel 리다이렉트([trace.py `_do_descend` to_pixel](../../../arbor/engine/trace.py#L477))는
    이 **진짜 `hypothesized failed`** 에 그대로 건다(object 에서 내려가면 grid.pixels — object 는 idx
    children 이 비어 필수).
...
- object-level 이 `colored-all`/`sim` 해킹·"원천차단" 주석 없이 "현재 (부분) program 을 verify 가
  실행 → G1 미생성 → 진짜 `hypothesized failed` → 하강" 으로 읽힌다.
